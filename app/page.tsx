...
export default function GiftPage() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      const now = new Date()
      const tripDate = new Date('2025-01-16T00:00:00')
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <HotelDetail icon={<Bed className="w-5 h-5" />} title="Habitación Deluxe" description="Cama king-size con vistas al jardín" />
              <HotelDetail icon={<Spa className="w-5 h-5" />} title="Spa Completo" description="Acceso ilimitado a las instalaciones del spa" />
              <HotelDetail icon={<Utensils className="w-5 h-5" />} title="Restaurante Gourmet" description="Cena romántica incluida" />
              <HotelDetail icon={<Wifi className="w-5 h-5" />} title="Wi-Fi Gratuito" description="Conexión de alta velocidad en todo el hotel" />
              <HotelDetail icon={<Dumbbell className="w-5 h-5" />} title="Gimnasio" description="Abierto 24/7 para mantenernos en forma" />
            </div>
            <button
              className="mt-6 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded w-full transition duration-300"
